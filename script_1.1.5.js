(function () {
  ...
  function buildTree(container, instance) {
    sap.ui.require([
      ...
      'sap/m/VBox'
                 ...
                 ScrollContainer, VBox) { // ★ ScrollContainer 인자 추가
      ...
      // ★ 렌더 후 oScroll이 남은 높이를 flex로 채우게 보정
  ...
  // ─────────────────────────────────────────────
  // Web Component (원본 그대로 — 중복 방지 체크 없음)
  // ─────────────────────────────────────────────
  class Main extends HTMLElement {
    ...
    connectedCallback () {
      // UI5는 Shadow DOM 미지원 → Light DOM 컨테이너에 렌더링 (원본 그대로)
      if (!this._container) {
